...
def _build_command(executable, width, height, config, hex_output, export_size):
    size_mode = int(_config_value(config, "size-mode", 3))
    ...
    if not bool(_config_value(config, "auto-tune", True)):
...
class Gif320SharpExportPlugin(Gimp.PlugIn):
    ...
    def _run_existing_preview_filter_dialog(self, image, drawable):
        ...
        def sync_preview(_changed_config, _pspec):
